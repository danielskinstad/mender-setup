...
func TestSetupInteractiveMode(t *testing.T) {
	...
	dev, err := io.ReadFile(config.DeviceTypeFile)
	assert.NoError(t, err)
...
func TestInstallDemoCertificateLocalTrust(t *testing.T) {
	...
	tdir, err := io.TempDir("", "mendertest")
	assert.NoError(t, err)
	...
	assert.NoError(t, err)
	crtSource, err := io.ReadFile(getMenderDemoCertPath())
	assert.NoError(t, err)

	crtInstall, err := io.ReadDir(DefaultLocalTrustMenderDir)
...
func checkCrtInstall(t *testing.T, cert string, demoCertContent []byte) {
	...
	contentBytes, err := io.ReadFile(cert)
